...
exec(open('/d2/studies/SmartSPIM/AT_RotationProject/' + sampleName + '_Stitched/clearMap_parameters_AT_'+sampleName+'.py').read())

if testing==True and testRegion=='all':
    raise NameError("Testing is true but no region specified.")
if not testing:
    removeBackgroundParameter['save']=None
    filterDoGParameter['save']=None
    detectCellShapeParameter['save']=None
#%%
...
topPerc = numpy.percentile(intensities[:,3], 99)
fig = mplt.figure()
vox = intensities[intensities[:,3]<topPerc][:,3]
mplt.hist(vox, bins=100)
mplt.show()
...
minFiltered = str(intensities[intensities[:,3]<minSize].shape[0])
maxFiltered = str(intensities[intensities[:,3]>maxSize].shape[0])
...
params = minFiltered + " cells below Min size: " + str(minSize) + "\n" + maxFiltered + " Cells above Max size: " + str(maxSize)
...
if testing:
    ...
    io.writeData(os.path.join(BaseDirectory, sampleName+'_cells_check_'+testRegion+'_'+str(minSize)+'-'+str(maxSize)+'px_'+time+'.tif'), data);
...
df_counts['name']=table['name'].astype(str)
...
df_counts_collapse['name']=table['name'].astype(str)
